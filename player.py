...
def search():
	try:
		...
		result = render_template("list.html", list=vk_response, current_song=current_song_title())
		return result
	except Exception as e:
		print e
...
def search_album():
	try:
		...
		for track in tracklist.findAll("track"):				
			query = { 'artist' : album_artist, 'title' : track.find(name="name").string, 'duration' : track.duration.string }
...
def find_play_range_worker(query_list):
	...
	for query in query_list:
		item = get_single_song(query["artist"], query["title"], query["duration"])
		if(item):			
			if first:
				player.addplay(item["href"], item["artist"], item["title"], item["duration"])
			else:
				player.add(item["href"], item["artist"], item["title"], item["duration"])
		first = False
...
def search_artist():
	try:
		...
		result = render_template("albumlist.html", list=album_search_results, artist=corrected_artist, current_song=current_song_title())
		return result
	except Exception as e:
		return e
...
def top():
	try:		
		...
		query_list = []		
		for track in tracklist.findAll("track"):				
			query = { 'artist' : corrected_artist, 'title' : track.find(name="name").string, 'duration' : track.duration.string }			
			query_list.append(query)
		find_play_range(query_list)			
		return fetch_playlist()
	except Exception as e:
	...
	try:
		...
		if mode == "list":
			...
			for item in channel.findAll("item"):
				...
				if p:
					list.append(p)				
			result = render_template("list.html", list=list, current_song=current_song_title())
			return result						
	except Exception as e:
		return e
